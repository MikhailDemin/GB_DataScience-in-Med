...
В качестве поля id в таблицах удобно использовать псевдо тип SERIAL
(псевдоним для типа ```BIGINT UNSIGNED NOT NULL AUTO_INCREMENT UNIQUE```).
Для информации: допустимые расположения файла my.cnf (или my.ini для Windows) описаны на странице официальной
документации: https://dev.mysql.com/doc/refman/8.0/en/option-files.html
Небольшая русскоязычная шпаргалка по утилите mysqldump:
https://mysqlru.com/mysql-database-administration/client-side-scripts/mysqldump.html
Решение присылайте, пожалуйста, в прикрепленных на портале SQL-скриптах (текстовые файлы с расширением *.sql). */